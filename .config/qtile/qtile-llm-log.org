#+title: llm-log quota widgets: z.AI and Codex
...
The labels are =z.AI= and =Codex=. Percentages mean USED. Each widget rotates its
...
The Codex meter is explicitly scoped to managed Codex account quota; it is not all ChatGPT web/voice/model usage.
