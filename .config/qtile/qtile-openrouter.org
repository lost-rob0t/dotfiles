...
Qtile directly starts a detached OpenRouter collector subprocess during
telemetry installation.  Widget polling never performs provider I/O and never
launches helpers: it only reads the collector's atomic local cache.  The
collector persists trusted provider buckets in a local SQLite history under
...
For the rotating text and graph-range widgets, left-click pauses automatic
rotation and advances once, right-click shows statistics, and middle-click
