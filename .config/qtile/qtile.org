...
* Plugins
** Qtile Gserver
#+begin_src python :tangle ./gserver-widgets/__init__.py
import asyncio
import threading
...
class RabbitMQMessageCounter(base.ThreadPoolText):
    defaults = [
        ('exchange_name', 'documents', 'Name of the RabbitMQ exchange'),
        ('routing_key', 'documents.new.#', 'Routing key for the exchange (empty string for all messages)'),
    ...
    def __init__(self, **config: Any):
        ...
        self.add_defaults(RabbitMQMessageCounter.defaults)
        self.queue = Queue()
    ...
    async def _update_message_count(self):
        try:
            ...
            channel.exchange_declare(exchange=self.exchange_name, exchange_type='topic', routing_key="documents.new.#")

            # Declare a queue, bind it to the exchange
            ...
            if self._last_reset is None or self._last_reset.date() != datetime.now().date():
                self.queue.queue.clear()
                self._last_reset = datetime.now()

            self.queue.put(message_count)
            connection.close()
        except AMQPConnectionError:
            # Handle RabbitMQ connection error
            self.queue.put("RabbitMQ error")

    def _update_loop(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        while True:
            loop.run_until_complete(self._update_message_count())

    def poll(self):
        return self.queue.get()

    def tick(self):
        message_count = self.poll()
        self.update(str(message_count))
#+end_src
