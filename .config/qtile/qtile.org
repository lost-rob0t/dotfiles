...
The report ranks keybind use, focused and managed applications, unmatched applications, common group destinations, layout changes, automatic routes, and group or screen usage. It also reports JSONL integrity. Malformed historical rotations are counted and skipped so older good telemetry remains usable; malformed records in the active log are reported separately. Use =--fail-on-active-corruption= when an active-writer integrity failure should produce exit status 2.
