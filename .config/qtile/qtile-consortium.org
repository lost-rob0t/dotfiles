#+title: Consortium observer widgets for Qtile
#+property: header-args:python :mkdirp yes :results none

* Contract

These widgets are read-only observers of Consortium's bounded desktop status
projection. They never start workers, query the JSONL control protocol, inspect
artifacts, or acquire model credentials. The default source is
=$XDG_STATE_HOME/consortium/qtile-status.json= and may be overridden at runtime
with =CONSORTIUM_QTILE_STATUS_FILE=.

Only the primary Qtile screen gets the two widgets. =CONS= shows active-run
count or the latest terminal outcome; =ADARD= shows the current phase or latest
terminal state. Right-click shows the bounded source details. A shared daemon
thread polls the atomic local file while widget rendering reads memory only.

* Bounded local status client
#+begin_src python :tangle consortium_status.py
...
#+begin_src python :tangle qtile_consortium.py
