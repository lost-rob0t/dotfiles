#+title: Qtile audio widget: default sink volume with an output dropdown
#+property: header-args:python :tangle ./qtile_audio.py :results none

Replaces the stock =Volume= widget, whose =amixer set Master= commands
cannot see PipeWire outputs.  This plugin talks to =pactl= and always
controls the *active* (default) sink:

- *Left click*: dropdown of every output; picking one runs
  =pactl set-default-sink= and pins it in a state file so the
  preference is restored on the next session.
- *Scroll up/down*: raise/lower the active sink volume by =VOLUME_STEP=.
- *Middle click*: toggle mute.
- The label shows an icon, the volume percent and the short sink name.

All =pactl= calls run in the poll thread or an executor; the Qtile event
loop never blocks.  The dropdown is a =Popup= subclass that maps click
rows back to sinks.
