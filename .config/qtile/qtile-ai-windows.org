...
Streamer mode guards the recorded center screen.  =STREAM_ALLOWLIST= is the
single source of truth for what may appear there: the Nyxt browser, the
sandboxed =StreamEmacs= frames, and OpenCode terminals.  Everything else is
blocked from the center screen while streamer mode is armed (see the Topics
& Streamer Mode section of qtile-ai.org).
