...
classifiers used by Auto mode.  It also loads the deterministic multi-monitor
focus policy and declarative workflow runtime before the topology-aware
desktop control module is installed, so config-load policy and workflow
activation can extend the runtime without modifying the control layer itself.
