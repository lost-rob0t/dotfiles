...
:DEPENDENCIES: Requires autostart.sh script in ~/.config/qtile/scripts/
:END:

** What Gets Started
- *startup_once*: Runs ONCE when Qtile first starts (per session)
- *startup*: Runs EVERY TIME Qtile restarts (Mod+Shift+R)
...
def start_always():
    """
    Execute on EVERY Qtile restart.
    Currently: Set cursor to default arrow (left_ptr).
    """
...
group_layouts = [
    ...
    "monadtall",  # 3: AI routing when Auto mode is enabled
    "monadtall",  # 4: Games and VMs
    "monadtall",  # 5: General
    "monadtall",  # 6: Video and media
    "monadtall",  # 7: General
    "monadtall",  # 8: File managers
    "monadtall",  # 9: Messenger and email
    "monadtall",  # 0: General
]
...
matching windows that were already open when Auto mode is enabled and repeats
the reconciliation once per second.
...
def apply_auto_grouping(window):
    ...
    if not auto_group_mode or not getattr(window, "group", None):
        return
    target_group = routed_group(window)
    if target_group and window.group.name != target_group:
...
def organize_existing_windows(qtile):
    ...
    for group in tuple(qtile.groups):
        for window in tuple(group.windows):
            target_group = routed_group(window)
            if target_group and target_group != group.name:
                window.togroup(target_group, switch_group=False)
...
def init_widgets_list():
    ...
    widgets_list = [
        ...
        # --- WEATHER ---
        widget.Wttr(
            format='%f %C',  # Temperature and condition
            location={f'@{myIp}': 'Home'},  # Use external IP for location
            units="u",  # Imperial units
...
# === SCREEN-SPECIFIC WIDGET LISTS ===

def init_widgets_screen1():
    """
    Primary monitor widget list.
    Includes system tray (only one tray per Qtile instance).
    """
...
def init_widgets_screen2():
    """
    Secondary monitor widget list.
    No system tray (can only have one).
    """
...
def init_screens():
    """
    Initialize screens with status bars.

    Creates one screen per physical monitor.
    Primary screen gets system tray, others don't.
    """
    return [
