...
- [[*Workspace Configuration][Workspace Configuration]] - Groups/virtual desktops + topic pool
...
- [[*Topics & Streamer Mode][Topics & Streamer Mode]] - Topic pool runtime and center-screen privacy
...
** Topic Registry & Generated Group Pool

Topics map groups to named subjects (projects, communications, media,
scratch space).  A topic is ~(name, label, default layout)~; each topic
generates one prefixed group =t:<name>= so the pool scales arbitrarily
without colliding with the static ="1".."0"= workspaces.  Add a line to
=DEFAULT_TOPICS= and a new workspace exists after a reload; new topics can
also be created at runtime through the Emacs dashboard or
=qtile cmd-obj -o cmd -f create_topic -a <name>=.

Every =group_names= consumer works on the extended set through
=all_group_names()= (static + topic groups + the streamer holding group).
=OwnedGroupBox= only renders groups with live windows, so hundreds of empty
topic groups cost nothing on the bar.  Topic groups are exempt from Auto
routing: windows placed in a topic stay there.
...
| Mod+g g          | Jump to topic (picker)    |
| Mod+g n / Mod+g p| Next/previous topic group |
| Mod+g t          | Toggle Emacs topics UI    |
| Mod+Shift+g      | Move window to topic      |
| Mod+s            | Toggle streamer mode      |
| Mod+Shift+s      | Streamer mode + recording |
| Mod+u            | Launch stream Emacs       |
...
* Topics & Streamer Mode
:PROPERTIES:
:DESCRIPTION: Topic runtime, IPC commands, keybindings, streamer privacy mode
:DEPENDENCIES: Requires 'groups', 'keys', 'mod', qtile_topics, qtile_streamer
:END:

** Topic runtime
The Emacs topics dashboard (=Mod+g t=) lists every topic with switch, send,
create, and close actions.  It is anchored to the center-bar Mara button
because keyboard flows have no trigger widget.  The jump/send pickers reuse
the workflow picker pattern: Emacs runs =completing-read= and returns the
chosen topic to Qtile; the picker runs on a worker thread and the result is
applied via =call_soon_threadsafe=, so the event loop never blocks.

Runtime topic management is exposed on the IPC root so Emacs and shell tools
can drive it (=lisp/qtile/qtile-topics.el= calls these):

| Command                                       | Action                        |
|-----------------------------------------------+-------------------------------|
| ~qtile cmd-obj -o cmd -f create_topic -a <n>~   | Register a new topic group    |
| ~qtile cmd-obj -o cmd -f remove_topic -a <n>~   | Close a topic (windows move)  |
| ~qtile cmd-obj -o cmd -f focus_topic -a <n>~    | Switch to a topic             |
| ~qtile cmd-obj -o cmd -f send_to_topic -a <n>~  | Move focused window to topic  |
...
(screen role =center=, resolved via =screen_roles()=).  It is OFF on every
Qtile start and never persists.  While armed, only allow-listed windows may
occupy the group shown on the center screen: Nyxt, the sandboxed
=StreamEmacs= (launched with =Mod+u=), and OpenCode terminals.  The
allow-list lives in =STREAM_ALLOWLIST= inside qtile-ai-windows.org.  Any
other window landing on center is moved to the =hold= group on another
screen — never killed — and enabling sweeps existing violations off center.
A config reload resets streamer mode to off, which is deliberate.

=Mod+Shift+s= additionally starts a center-screen recording through
=screen-capture record-screen= when arming and stops it when disarming
(streamer only stops recordings it started itself).  The X11 recorder
captures the monitor under the pointer, so keep the pointer on the center
screen when arming.  Both recording calls spawn without waiting.
