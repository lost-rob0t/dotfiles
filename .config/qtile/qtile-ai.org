...
#+author: lost-rob0t
#+property: header-args:python :tangle ./config.py :mkdirp yes :results none
#+startup: overview

* Source of truth

This Org file is the authoritative source for =config.py=. Edit this file first,
tangle it, and commit both files together.

* Browser launch behavior

- =Super+w= opens Brave in a new window.
- =Super+F1= opens Brave in a new window.
- Brave is launched through =~/.nix-profile/bin/brave= so Qtile does not depend
  on a display-manager inherited =PATH= for this binding.
