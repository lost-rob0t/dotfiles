#+title: Pinger
#+property: header-args:python :tangle ./pinger.py :mkdirp yes :tangle-mode (identity #o755) :results none

* Overview

This file is the source of truth for the qtile autostart host aliveness
monitor. It tangles to =.config/qtile/scripts/pinger.py=, which is launched
from =autostart.sh= and emits a dunstify alert whenever a configured host
stops responding.

Hosts are defined in =~/.config/hosts.toml= (TOML, not the legacy plain
=text list). Each entry picks a =method= of either =ping= (ICMP, default)
or =ssh= (non-interactive =ssh ... true= probe). SSH probes can specify
=user=, =port=, =key=, and =timeout=.

Tangle this file whenever the pinger changes:

: emacs --batch -l org --eval "(let ((org-confirm-babel-evaluate nil)) (org-babel-tangle-file \".config/qtile/scripts/pinger.org\"))"
