#+TITLE: Doom-equivalent native desktop UX
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :comments no :padline yes :mkdirp yes

This is the native desktop UX contract. It recreates the visible and
muscle-memory parts of the enabled Doom modules without loading Doom itself.
Doom remains the desktop default until this profile passes parity checks.
One time a human wrote this. Doom merely industrialized the parentheses.

* Runtime
#+begin_src emacs-lisp :tangle autoload/doom-ux.el
