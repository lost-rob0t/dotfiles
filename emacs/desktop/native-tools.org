#+TITLE: Native Doom Tool Compatibility
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :comments no :padline yes :mkdirp yes

Runtime equivalents for enabled Doom tool/app modules whose behavior is more
than simply installing a package.  Keep these functions Doom-compatible where
that preserves existing muscle memory or private Lisp callers.

* Runtime
#+begin_src emacs-lisp :tangle autoload/native-tools.el
