#+TITLE: Native Doom Session Compatibility
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :comments no :padline yes :mkdirp yes

Doom's =SPC q= tree is muscle memory too.  Native Emacs keeps the same quit,
session and restart shape; config sync is retained as a native extension on
=SPC q c= instead of stealing Doom's restart binding.

* Runtime
#+begin_src emacs-lisp :tangle autoload/native-session.el
