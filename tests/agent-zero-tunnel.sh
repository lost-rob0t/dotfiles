...
grep -Fxq 'ServerAliveInterval=5' "$SSH_CAPTURE"
grep -Fxq 'ServerAliveCountMax=6' "$SSH_CAPTURE"
grep -Fxq 'unseen@op.example' "$SSH_CAPTURE"

: > "$SSH_CAPTURE"
A0_REMOTE_PORT=50080 A0_LOCAL_PORT=50080 bash "$tunnel" >/dev/null
grep -Fxq 'flake' "$SSH_CAPTURE"

if A0_REMOTE_PORT=70000 bash "$tunnel" flake >/dev/null 2>&1; then
