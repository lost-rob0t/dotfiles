...
grep -Fq 'app_mapping(agenda,' "$config" || fail "org agenda workflow missing"
grep -Fq 'verb_intent(clear, open, 1).' "$config" || fail "clear-backlog phrase mapping missing"
grep -Fq 'app_mapping(backlog,' "$config" || fail "backlog clear mapping missing"
grep -Fq 'nsa/org-agenda-clear-backlog' "$config" || fail "backlog clear does not invoke the Doom agenda command"
grep -Fq 'app_mapping(capture,' "$config" || fail "org capture workflow missing"
