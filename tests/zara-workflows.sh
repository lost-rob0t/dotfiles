...
grep -Fq 'enableAll = true;' "$desktop" || fail "desktop profile must install the complete pinned Zara plugin registry"
! grep -Fq 'registry = [ "zara-discord" "zara-persona" ];' "$desktop" || fail "desktop profile must not regress to a hand-picked Zara plugin subset"
grep -Fq 'cfg.plugins.enableAll' "$repo_root/nix/home-manager/mods/zara.nix" || fail "Zara Home Manager module does not implement full-registry selection"
grep -Fq 'allRegistryPluginNames' "$repo_root/nix/home-manager/mods/zara.nix" || fail "Zara Home Manager module does not derive all plugins from the pinned registry"
