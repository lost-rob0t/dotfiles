...
grep -Fq 'enable = false;' "$desktop" || fail "desktop profile must keep the Qwen3-TTS GPU service disabled at login"
grep -Fq 'backend = "rocm";' "$desktop" || fail "desktop profile does not retain the intended AMD ROCm backend"
