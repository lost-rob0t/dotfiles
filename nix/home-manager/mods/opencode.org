...
Command templates remain deliberately small.  They select a reusable skill and
pass OpenCode's placeholders through literally.  =globalSkills.sourceCheckout=
records the editable global skills checkout explicitly; it is metadata for the
prompts, not a guessed discovery path or a Home Manager-owned checkout.
...
the generic skill dispatch is insufficient.  To add a model-selecting workflow,
add it to =modelWorkerCommands= and implement model routing in the shared worker
skill, never in command metadata.
