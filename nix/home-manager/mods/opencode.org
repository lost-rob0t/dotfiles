...
other provider: llm-log gains a =starintel= upstream and OpenCode points at
=<proxy>/starintel/v1=, so the bearer key stays in a private key file read
through OpenCode's ={file:}= placeholder and never enters the Nix store or
Git.  Model entries are declarative metadata (display name, context and output
