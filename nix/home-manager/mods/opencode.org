#+TITLE: OpenCode Home Manager Module
#+PROPERTY: header-args:nix :tangle opencode.nix :mkdirp yes

* Architecture
This is the canonical source for the local OpenCode Home Manager module.  The
upstream Home Manager module owns OpenCode itself; this module adds dotfiles
policy, the Outrun theme, proxy routing, and typed global commands without
claiming project-local configuration.

#+begin_example
opencode.commands (typed declarations)
              |
              v
...
project checkout/.opencode/commands/*  (independent, untouched)
#+end_example

Command templates remain deliberately small.  They name the slash command and
reusable skill, state the command intent, and pass OpenCode's placeholders
through literally as optional command input.  A bare command never turns empty
=$ARGUMENTS= into a punctuation-only pseudo-goal; stateful no-argument commands
recover repository/session context through the selected skill.  The
=globalSkills.sourceCheckout= value records the editable global skills checkout
explicitly; it is metadata for the prompts, not a guessed discovery path or a
Home Manager-owned checkout.

The module also owns the optional =opencode.web= persistent web workspace:
=web.enable=, =web.hostname=, and =web.port= drive Home Manager's native web
service support plus the =opencode-attach= and =opencode-web-open= helpers; see
the desktop wiki page for the deployed workspace.

The optional =opencode.starintelLlm= provider publishes the StarIntel local
inference service (llama-swap behind =llm.starintel.actor=) as a first-class
OpenCode provider.  Requests follow the same llm-log capture plane as every
other provider: the transport is a =starintel= llm-log upstream declared where
the proxy module is imported (=llm.nix=), and OpenCode points at
=<proxy>/starintel/v1=.  This module never touches =services.llm-log= itself
because configurations that enable OpenCode do not necessarily import the
proxy module.  The bearer key stays in a private key file read through
OpenCode's ={file:}= placeholder and never enters the Nix store or Git.
Model entries are declarative metadata (display name, context and output
limits, vision attachment); each one becomes a selectable
=starintel-llm/<model>= reference.  Embeddings-only models are registered for
completeness but cannot hold conversations.

The reusable =opencode-worker= follows a Unix filter contract: stdin is the
complete child task, stdout is usable model output, stderr is diagnostics, and
status is success or failure.  Logical model names resolve against the live
OpenCode catalog.  Model-selected children in RAGE, ADADR/ARADR, GRILL, review,
research, delegation, and fleets use that wrapper; ordinary model-agnostic
subtasks may remain native.  Worker concurrency, retries, and prompt size are
bounded (Linux caps one process argument near 128 KiB, so oversized prompts are
rejected before launch), parallel mutation requires isolated Git worktrees, and
worker results are emitted through plain sized writes so downstream closure is
observable.

RAGE remains the existing issue-to-merge workflow and ADADR remains its actual
architecture loop; =/aradr= first discovers whether a project defines a distinct
ARADR research pipeline rather than inventing one.  GRILL is an evidence-based
cross-examination, not a synonym for normal review.  Merge-on-green supports
GitHub and Forgejo and invalidates green evidence whenever the PR head changes.

To add a command, add one =commandSpecs= entry and a special template only when
the generic skill dispatch is insufficient.  Special templates must preserve
the command-identity and optional-argument contract.  To add a model-selecting
workflow, add it to =modelWorkerCommands= and implement model routing in the
shared worker skill, never in command metadata.
