...
;; Keywords: tools, cashier, retail, calculator
;; URL: https://github.com/lost-rob0t/.dotfiles

;;; Commentary:

;; Cash provides a fast cashier drop calculator for retail work.
;; Designed for Android Emacs with persistent keyboard visibility.

;; Key features:
;; - Fast cash counting with multiple input methods
;; - Automatic drop calculation (keep $100 in drawer)
;; - Emergency 50s/100s checking
;; - Session logging and tracking
;; - Android-optimized interface

;; Usage:
;;   M-x cashier-android-dashboard  ; Main dashboard
;;   M-x cashier-android-quick      ; Quick count
;;   M-x cashier-super-quick        ; Individual prompts
;;   M-x cashier-emergency-50-100   ; Emergency check

;;; Code:

(defgroup cashier nil
  "Fast cashier drop calculator."
  :group 'tools
  :prefix "cashier-")

(defcustom cashier-drawer-target 100
  "Target amount to keep in drawer."
  :type 'integer
  :group 'cashier)

(defcustom cashier-denominations
  '((20 . "20s")
    (10 . "10s")
    (5 . "5s")
    (1 . "1s")
...
(defun cashier-android-quick ()
  ...
  (interactive)
  (let ((input (read-string "Enter: 4 20s, 0 10s, 10 5s, 40 1s (or just numbers): ")))
    (let* ((counts (cashier-parse-input input))
           (total (cashier-count-cash counts))
           (drop-amount (cashier-calculate-drop total)))

      (with-current-buffer (get-buffer-create "*Quick Count*")
        (erase-buffer)
        (insert (format "QUICK COUNT\n===========\n\n"))
...
(defun cashier-quick-numbers ()
  "Quick input with just comma-separated numbers (20s,10s,5s,1s,qtrs)."
  (interactive)
  (let ((input (read-string "Numbers only (20s,10s,5s,1s,qtrs): ")))
    (let* ((numbers (mapcar #'string-to-number (split-string input "[, ]" t)))
      ...
      (message "4×$20 + 0×$10 + 10×$5 + 40×$1 = %s | Drop: %s"
               (cashier-format-money total)
               (cashier-format-money drop-amount))
