...
builds all four applications from source; it calls the same host-side Emacs
...
Both requirements are mandatory: all four APK manifests must declare the
...
# Build Termux, Termux:API and Termux:Widget as in the paired workflow, or
...
python3 android/build.py sign \
  ...
  --widget-apk /path/to/termux-widget.apk \
