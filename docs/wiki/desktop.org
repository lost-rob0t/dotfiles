...
The Qtile Auto mode button reconciles windows once per second while it is
enabled.  AI-related windows go to workspace 3, Discord goes to workspace 9,
games go to workspace 4, and video players or video-site tabs go to workspace
6.  This includes ChatGPT Desktop, AI tabs in Brave, T3 Code,
Emacs LLM/OpenRouter/MCP buffers, OpenCode or Claude Code Terminator windows,
Steam/PrismLauncher/Minecraft/War Thunder, and YouTube/Twitch tabs.
...
one-second pass uses Qtile's event loop timer and performs no subprocess or
network work.

The Bash =opencode= launcher opens a dedicated Terminator window.  Pass
=--topic=TOPIC= (or set =OPENCODE_CHAT_TOPIC=TOPIC=) to get a title of the
form =Opencode - TOPIC=; remaining arguments go to OpenCode.  =oc= is a
short alias for the same launcher.
