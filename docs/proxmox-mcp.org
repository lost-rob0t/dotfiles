...
- ProxmoxMCP-Plus pinned as a flake input (=proxmox-mcp-plus= in
  =flake.nix=, =flake = false= so the raw source is used). The launcher
  runs =uvx --from <flake source> proxmox-mcp-plus=, so the build is
  reproducible and =flake.lock= pins the exact revision. No manual
...
- New nix module =nix/home-manager/mods/proxmox-mcp.nix= providing a
  ...
  =~/.config/proxmox-mcp/proxmox-mcp.env= (kept *outside* git) and then
  runs =uvx= against the flake-pinned source.
...
- =flake.nix= (new =proxmox-mcp-plus= input)
