...
Make Lish the default interactive shell while keeping Bash as the recovery/login shim.
The shared handoff helper is also consumed by Home Manager so the policy has one
implementation.
#+begin_src sh :tangle .bashrc
...
*** OpenCode
OpenCode is invoked directly from the current terminal.  =oc= is a short alias
for the executable.
#+begin_src sh :tangle .bashrc
alias oc='opencode'
#+end_src
