...
Load the shared git synchronization helper in interactive shells.  Qtile uses the
same helper directly, so the shell command and keybinding have one implementation.
#+begin_src sh :tangle .bashrc
