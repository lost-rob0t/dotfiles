...
Canonical source for the Android/Termux bootstrap.  The generated
=bootstrap-termux.sh= detects the phone environment, upgrades the rolling Termux
userspace, installs Git/Emacs/Stow and the native language toolchain, provisions
the glibc compatibility guest for AI tools and Nix, installs the full Termux
Doom profile, exposes a Doom-free Org-first Emacs launcher, and creates
Termux:Widget shortcuts.

The native Android Emacs APK is a separate GUI application.  The paired Android
release build uses the same shared user ID/signing key as Termux, so native
Emacs can execute the Git installed here through the Termux prefix without
injecting Termux libraries into Emacs via =LD_LIBRARY_PATH=.

Nix deliberately lives inside the Linux/PRoot boundary instead of directly in
the Android Termux prefix.  This follows the same constraint that motivates
Nix-on-Droid: Android does not provide the filesystem/runtime assumptions of a
normal Nix host.  The bootstrap exposes =nix=, =nix-shell=, =nix-env=,
=nix-store=, and =nix-channel= as normal Termux commands which dispatch into the
guest.  Flakes and =nix-command= are enabled.

* Bootstrap

#+begin_src sh :tangle bootstrap-termux.sh
