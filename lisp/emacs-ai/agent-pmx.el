;;; demo-gptel-config.el --- Demo Gptel Config  -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;; These are the settings used in the demo for Retreival Augmented Macros.  It's
;; just a bunch of function definitions and a use package expression at the end.
;; You can copy-paste this, set up an OpenAI token.  The commit that adds this
;; file contains a draft of setting up GPTel.
...
If the user says call it, always call it.")))
  ...
  ;; TODO pixel scrolling is still pretty experimental.  Can look nice.  When it
  ;; works.  The input by gptel is done inside a save-excursion form.  I've seen
  ;; some max-eval depth exceeded depending on the approach.
  (defun pmx--auto-scroll ()
    (when-let* ((win (get-buffer-window (current-buffer) 'visible)))
      (with-selected-window win
        (let ((height (window-body-height win t))
              (text-size (window-text-pixel-size
                          win (window-start win) (point))))
          (when (> (cdr text-size) (* height 0.75))
            (unless (or (looking-back "^#\\+end_tool") (looking-at "^#\\+end_tool")
                        (looking-back "^```$") (looking-at "^```$"))
              (scroll-up-line)))))))

  (add-hook 'gptel-post-stream-hook #'pmx--auto-scroll)
