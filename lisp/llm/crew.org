#+TITLE: ARADR crews and retained Prolog knowledge
#+PROPERTY: header-args :mkdirp yes :comments no :padline no

* Ownership
This is the canonical source for the generated crew library. It extends the
existing OpenCode workspace; it does not replace Consortium or create a parallel
expert engine. Supporting generated policy, transport, skill and retention tools
are owned by [[file:crew-support.org][crew-support.org]].

Entry: =M-x +crew/workspace= or =M-x crew-open= after =(require 'crew)=.
See [[file:../../docs/wiki/crew.org][crew definitions and roles]] and
[[file:../../contracts/zara-crew-v1/contract.md][ZARA-CREW/1]] for semantics and limitations.

* Preservation
Prior Prolog bytes and paths remain Git tracked. New incompatible rules require
new versioned source/output paths, not destructive retangling over past modules.
DotfilesExpert re-exports the existing project KB rather than copying it.

* Verification
Python tests use real temporary Git repositories. ERT exercises the existing
actor registry and actual SWI-Prolog process bridge. Live OpenCode tool discovery,
authentication and remote expert integration are separate gates, never mock green.

* Crew library
#+begin_src emacs-lisp :tangle crew.el
