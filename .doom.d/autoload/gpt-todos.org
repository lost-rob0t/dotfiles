...
The complete live Org workspace is synchronized after a saved =.org= file below
=GPT_TODOS_NOTES_DIR= (default =~/Documents/Notes/org/=).  The historical agenda
subtree keeps its =GPT_TODOS_ORG_DIR= mapping for Todo Manager compatibility.
The work happens asynchronously so Git/network work never blocks Emacs.  This
autoload also makes the full workspace the Org-roam graph and exposes Org QL
views for shared agent memory and inventory.
