...
#+end_src
** Zara
The native Emacs client is shipped by the Zara repository and pinned to the
exact reviewed client commit.  Doom only installs =emacs/zara.el=; Zara's
runtime stays in the normal Zara/Nix deployment.
#+begin_src emacs-lisp
(package! zara
