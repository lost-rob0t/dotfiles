...
#+end_src
** Zara
The native Emacs client and =ZARA-EMACS/1= semantic bridge are shipped by the
Zara repository and pinned to the exact reviewed client commit. Doom installs
only =emacs/zara.el=; the canonical Zara runtime stays in the normal Nix deployment.
#+begin_src emacs-lisp
(package! zara
